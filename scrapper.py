...
def write_to_file(country_code, country_data):
    ...
    with open(f"{OUTPUT_DIR}/{country_code}_category_id_{datetime.now().strftime('%Y%m%d%H%M%S')}.csv", "a", encoding='utf-8') as file:
        for row in country_data:
            file.write(f"{row}\n")


def write_to_s3(country_code, country_data, bucket_name):
    print(f"> Writing {country_code} data to S3...")
    country_data.pop(0)
    ...
    for row in country_data:
        s3.put_object(
            Bucket=bucket_name,
            Key=f"youtube/raw_data/{country_code}_category_id_{datetime.now().strftime('%Y%m%d%H%M%S')}.csv",
...
def get_data():
    ...
    for country_code in COUNTRY_CODES:
        country_data[country_code] = [",".join(HEADER)] + get_pages(country_code)
        write_to_file(country_code, country_data[country_code])
